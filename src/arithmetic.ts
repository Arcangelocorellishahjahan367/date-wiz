 ...
 */
export function subtract(date: DateInput, amount: number, unit: DurationUnit): Date {
...
export function diff(
  ...
  switch (unit) {
    case 'milliseconds': case 'ms': return deltaMs;
    case 'seconds': case 's': return deltaMs / 1_000;
    case 'minutes': case 'm': return deltaMs / 60_000;
    case 'hours':   case 'h': return deltaMs / 3_600_000;
    case 'days':    case 'd': return deltaMs / 86_400_000;
    case 'weeks':   case 'w': return deltaMs / 604_800_000;
    case 'months':  case 'M': return deltaMs / 2_592_000_000;
    case 'years':   case 'y': return deltaMs / 31_536_000_000;
    default: return deltaMs;
  }
}
 ...
 */
export function startOf(date: DateInput, unit: 'year' | 'month' | 'week' | 'day' | 'hour' | 'minute'): Date {
  const d = clone(toDate(date));
  switch (unit) {
    case 'year':   d.setMonth(0); // falls through
    case 'month':  d.setDate(1); // falls through
 ...
 */
export function endOf(date: DateInput, unit: 'year' | 'month' | 'week' | 'day' | 'hour' | 'minute'): Date {
